...
impl MeasurementsResponse {
    pub fn get_chunk(
        ...
        let response_size = self.response_size(measurements)?;
        ...
        {
            let signature =
                self.l1_signature(self.asym_algo, shared_transcript, session_info, cert_store)?;
    ...
    fn response_variable_fields(
        ...
        let len = self.encode_response_variable_fields(&mut trailer_buf)?;
        Ok((trailer_rsp, len))
    }

    fn encode_response_variable_fields(&self, buf: &mut MessageBuf<'_>) -> CommandResult<usize> {
        // Encode the nonce
    ...
    fn l1_signature(
        ...
        let _ = self.encode_l1_signature(
            asym_algo,
            transcript,
            session_info,
            cert_store,
    ...
    fn response_size(&self, measurements: &mut SpdmMeasurements<'_>) -> CommandResult<usize> {
...
pub(crate) fn generate_measurements_response<'a>(
    ...
    let rsp_len = match rsp_ctx.response_size(&mut ctx.measurements) {
        Ok(len) => len,
