...
fn disable_cpu_idle(cpu: usize, target_host: Option<&str>) -> Result<()> {
    run_command(
        target_host,
        &format!("sudo echo 1 > /sys/devices/system/cpu/cpu{cpu}/cpuidle/state1/disable"),
    )?;
    let state = run_command_with_output(
        target_host,
        &format!("sudo cat /sys/devices/system/cpu/cpu{cpu}/cpuidle/state1/disable"),
    )?;
...
pub(crate) fn fpga_entry(args: &Fpga) -> Result<()> {
    match args {
        ...
        Fpga::Test { target_host } => {
            ...
            // Clear old test logs
            let _ = run_command(target_host.as_deref(), "(sudo rm /tmp/junit.xml || true)");
