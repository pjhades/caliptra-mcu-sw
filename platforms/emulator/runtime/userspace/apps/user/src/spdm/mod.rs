...
use caliptra_mcu_spdm_lib::transport::common::SpdmTransportSync;
...
pub(crate) fn spdm_task() {
    ...
    let shared_cert_store = match initialize_cert_store() {
        Ok(cert_store) => SharedCertStore::new(cert_store),
        Err(e) => {
            writeln!(
                console_writer,
    ...
    spdm_mctp_responder(shared_cert_store);

    // TODO: Handle DOE transport.
    //#[cfg(feature = "doe")]
